...
(function () {
  ...
  // ====== Estado ======
  const FACETS = ['categoria', 'tecnologia', 'material', 'cores', 'gramatura', 'largura', 'composicao', 'ligamento', 'aplicacao'];
  const state = { search: '' };
  ...
  function valoresDoFaceta(facet, p) {
    switch (facet) {
      ...
      case 'tecnologia': return p.tec || [];
      case 'material':   return p.mat || [];
  ...
  function closeFiltros() {
    ...
    const tb = root.querySelector('[data-toggle-filtros]');
  ...
  root.addEventListener('click', (e) => {
    ...
    if (swatch) {
      ...
      const sel = state.cores.has(value);
      swatch.classList.toggle('is-active', sel);
      swatch.setAttribute('aria-pressed', sel ? 'true' : 'false');
      render();
      return;
    }
    ...
    // Fechar a gaveta de filtros no mobile (botão ✕ e "Ver resultados")
    if (e.target.closest('[data-fechar-filtros]')) {
      closeFiltros();
      return;
    }
